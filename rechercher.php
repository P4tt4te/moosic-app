...
$recherche = NULL;
$nbWebradios = $bdd->prepare('SELECT COUNT(*) AS nb_webradios FROM Radio'); // On compte le nombre de résultat pour faire un afficheur
$nbWebradios -> execute();
$totalRadio = $nbWebradios -> fetch(PDO::FETCH_ASSOC);
if(isset($_GET['rechercher']) AND !empty($_GET['rechercher'])){
   ...
   $rechercheRadio = $bdd->prepare('SELECT * FROM Radio WHERE (nomRadio LIKE ? OR categorie LIKE ?)'); // On cherche dans la bdd les valeurs qui correspondent aux ?
   $rechercheRadio->execute(array('%'.$recherche.'%','%'.$recherche.'%')); // les % servent à faire en sorte qu'on ne s'occupe pas des valeurs avant ou après
   $nbRecherche = $bdd->prepare('SELECT COUNT(*) AS nb_recherche FROM Radio WHERE (nomRadio LIKE ? OR categorie LIKE  ?)'); // On compte le nombre de résultat pour faire un afficheur
   $nbRecherche->execute(array('%'.$recherche.'%','%'.$recherche.'%'));
...
<?php if(isset($_SESSION['idUtilisateur'])){ ?>
  ...
  <head>
      ...
      <link rel="stylesheet" href="style-rechercher.css" type="text/css" media="screen" />
    ...
    <main>
        <div class="full-container">
            <div class="div-recherche">
                <img src="https://cdn.glitch.com/635f7807-931c-4359-955f-e5a30b567eb1%2Ffleche.svg?v=1622465156072"
                    class="flecheRec" />
                <div class="search-container">
                    <p>
                        Cherchez et trouvez votre bonheur dans notre catalogue de <?php echo($totalRadio["nb_webradios"]); ?> webradios.
                    </p>
                    <form method="GET" class="form-recherche" action="">
                        <input class="barre-recherche" type="search" placeholder="Rechercher..." name="rechercher" />
                        <input class="bouton-valider valider" type="submit" value="Valider">
                    </form>
                </div>
            </div>
            <img src="https://cdn.glitch.com/635f7807-931c-4359-955f-e5a30b567eb1%2Fpersonnage.svg?v=1622465156236"
                class="personnage-guitare" />
        </div>
        <section class="section-resultats">
            <div class="nombre-resultats">
                    <?php if(isset($nbRecherche)){  // boucle qui parcours le tableau pour indiquer le nombre de recherche
                        $ligne2 = $nbRecherche->fetch(PDO::FETCH_ASSOC);?>


                        <p class="chiffresRes bold"> <?php echo($ligne2['nb_recherche']); ?> </p> <p class="lettresRes bold"> résultat(s) </p>
                        <?php
                        }else{
                            echo null;}?>
            ...
            <div class="swiper-like">
                ...
                <div class="liste-webradios-likees swiper-container">
                    ...
                    <?php if($recherche != NULL){
                     while($ligne = $rechercheRadio->fetch(PDO::FETCH_ASSOC)) { // boucle qui parcours le tableau pour afficher les radios?>
                    <div class="case-radio swiper-slide"><a href = "https://la-projets.univ-lemans.fr/~mmi1pj03/profil-radio.php?id=<?php echo $ligne['idRadio'];?>"><img class="logo-radio" alt="logo radio" src="./images/<?php echo($ligne['photoRadio']); ?> "></a> <span class="nomRadRec"><?php echo($ligne['nomRadio']);?></span> <span class="catRadRec"><?php echo($ligne['categorie']);?></span></div>
                    <?php }
                  }?>
                </div>
            </div>
